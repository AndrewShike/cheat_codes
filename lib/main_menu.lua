...
function main_menu.init()
  ...
  elseif menu == 2 then
    ...
    if key1_hold then
      ...
      screen.move(88,60)
      local rate_options = {eight .. " s", (eight * 2) .. " s", (eight * 4) .. " s"}
      screen.text(rate_options[params:get"live_buff_rate"])
      screen.move(111,60)
      screen.level(3)
      screen.text(string.format("%0.f",util.linlin(rec.start_point-(eight*(rec.clip-1)),rec.end_point-(eight*(rec.clip-1)),0,100,(poll_position_new[1] - (eight*(rec.clip-1))))).."%")
    else
      screen.line_width(1)
      for i = 1,3 do
      ...
      for i = 1,3 do
        screen.level(page.loops_sel == i-1 and 15 or 3)
        local start_to_screen = util.linlin(1,(eight+1),15,120,(bank[i][bank[i].id].start_point - (eight*(bank[i][bank[i].id].clip-1))))
        screen.move(start_to_screen,24+(15*(i-1)))
        screen.text("|")
        local end_to_screen = util.linlin(1,(eight+1),15,120,bank[i][bank[i].id].end_point - (eight*(bank[i][bank[i].id].clip-1)))
        screen.move(end_to_screen,30+(15*(i-1)))
        screen.text("|")
        local current_to_screen = util.linlin(1,(eight+1),15,120,(poll_position_new[i+1] - (eight*(bank[i][bank[i].id].clip-1))))
        screen.move(current_to_screen,27+(15*(i-1)))
        screen.text("|")
      end
      screen.level(page.loops_sel == 3 and 15 or 3)
      local recording_playhead = util.linlin(1,(eight+1),15,120,(poll_position_new[1] - (eight*(rec.clip-1))))
      if rec.state == 1 then
      ...
      local recording_start = util.linlin(1,(eight+1),15,120,(rec.start_point - (eight*(rec.clip-1))))
      screen.move(recording_start,66)
      screen.text("|")
      local recording_end = util.linlin(1,(eight+1),15,120,rec.end_point - (eight*(rec.clip-1)))
